...
const ReviewClassCardUser = ({ title, tutor, src, rating, setRating }) => {
  ...
  return (
    <>
      <SectionMiddle>
        <ProfileSquare size={80} border={8} src={src} />
        <CardInfoWrap type="center">
          <ReviewCardTheme>{title}</ReviewCardTheme>
          <ClientName>{nameFiltering(tutor)} 전문가님</ClientName>
        </CardInfoWrap>
      </SectionMiddle>
      <ReviewWrap>
        <ReviewTheme>{nameFiltering(tutor)} 전문가님에 대한 별점</ReviewTheme>
        <StarWrap>
          {[1, 2, 3, 4, 5].map((index) => {
            return (
              <StarRating
...
export default ReviewClassCardUser;
