...
import { TutorAuthenticate } from "../../../SignUp/SignUp";
...
const EditTutorInfo = ({ title }) => {
  const history = useHistory();
  const { loading: queryLoading, data } = useQuery(GET_MY_TUTOR_INFO, {
    onCompleted: (d) => {
      if (d.getMyTutorInfo.ok) {
        setAvatarPreview(d.getMyTutorInfo.user.avatar);
        setPhoneNumber(d.getMyTutorInfo.user.phoneNumber);
        const splitAddress = d.getMyTutorInfo.user.address.split("||");
        setAddress1(splitAddress[0]);
        setAddress2(splitAddress[1]);
        const splitWorkingArea = d.getMyTutorInfo.user.workingArea.split("||");
        setWorkingCity(splitWorkingArea[0]);
        setWorkingTown(splitWorkingArea[1]);
      }
    },
  });
  ...
  const onClickMutation = () => {
    updateUserMutation({
      variables: {
        ...
        licenseImage: licenseImage ? licenseImage : undefined,
      },
    });
  };
  ...
  // const onAddProfileImg = (e) => {
  //   const imgSrc = "/img/" + e.target.files[0].name;
  //   if (profileImg !== imgSrc) {
  //     setProfileImg(imgSrc);
  //   } else {
  //     setProfileImg(sampleTutor.profileImg);
  //   }
  // };
  ...
  return (
    <>
      {!queryLoading && data?.getMyTutorInfo?.ok && (
        <>
          ...
          <Container>
            ...
            <CustomInput
              type={"name"}
              name={"이름"}
              value={data.getMyTutorInfo.user.name}
              disabled={true}
            />
            <EmailInput
              marginTop={"32px"}
              value={data.getMyTutorInfo.user.email}
              disabled={true}
            />
            <PasswordInput
              marginTop={"32px"}
